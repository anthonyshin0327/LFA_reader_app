...
if uploaded_file:
    ...
    st.image(ruler_layer, caption="ROI Preview with Pixel Ruler", use_column_width=False, width=800)
    ...
    for idx, (x1, y1, x2, y2) in enumerate(strip_coords):
        ...
        bg = np.mean(background_pixels) if background_pixels.size > 0 else 0
        bgsub = intensity - bg
        ...
        for peak in peaks:
            ...
            peak_bgsub = abs(intensity[peak] - bg)
            if label == "Test":
